...
export default class SearchRecent extends Component {
  ...
  render() {
    ...
    let { open, focusedItem } = this.props
    let { searches } = this.state

    if (!searches.length) return null

    searches = searches.map((search,i) => {
      return <a href="" key={i} className={"a-search__form__recent__i" + (i === focusedItem ? 'a-search__form__recent__active' : '')} onClick={this.onSelect.bind(this, search)} >{ search }</a>
    })
