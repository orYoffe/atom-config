...
export default class ASearchPopout extends Component {
  ...
  render() {
    ...
    dataNav.push({
      link: <Link to={`/search/${query}/all`} onClick={saveSearch.bind(null, query)}>{this.trans.all && this.trans.all.toUpperCase()}</Link>,
      content: (
        <SearchAll
          videos={videos}
          lives={lives}
    ...
    if(videos && videos.data && videos.data.length){
      dataNav.push({
        link: <Link to={`/search/${query}/videos`} onClick={saveSearch.bind(null, query)}>{this.trans.videos && this.trans.videos.toUpperCase()}</Link>,
        content: (
          <SearchVideos
            videos={videos}
            fetching={fetching}
    ...
    if(users && users.data && users.data.length){
      dataNav.push({
        link: <Link to={`/search/${query}/channels`}  onClick={saveSearch.bind(null, query)}>{this.trans.channels && this.trans.channels.toUpperCase()}</Link>,
        content: (
          <SearchChannels
            users={users}
            fetching={fetching}
    ...
    if(playlists && playlists.data && playlists.data.length){
      dataNav.push({
        link: <Link to={`/search/${query}/playlists`} onClick={saveSearch.bind(null, query)}>{this.trans.playlists && this.trans.playlists.toUpperCase()}</Link>,
        content: (
          <SearchPlaylists
            playlists={playlists}
            fetching={fetching}
