...
import toUpperCase from 'components/search/toUpperCase.jsx'
...
export default class ASearchPopout extends Component {
  ...
  render() {
    ...
    // TODO Localization
    let dataNav = [
      { link: <Link to={`/search/${query}/all`}>{this.trans.all && toUpperCase(this.trans.all)}</Link>, content: <SearchAll videos={videos} lives={lives} playlists={playlists} users={users} query={query}/> },
