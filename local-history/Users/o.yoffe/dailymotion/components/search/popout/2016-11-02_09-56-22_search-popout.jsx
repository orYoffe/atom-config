...
export default class ASearchPopout extends Component {
  ...
  render() {
    ...
    if(users && users.data && users.data.length){
      dataNav.push({
        link: <Link to={`/search/${query}/channels`}  onClick={navigate.bind(this, query, `/search/${query}/channels`, this.changeCurrentPaging.bind(this, users.paging))}>{this.trans.channels && this.trans.channels.toUpperCase()}</Link>,
        content: (
          <SearchChannels
            users={users}
            fetching={fetching}
