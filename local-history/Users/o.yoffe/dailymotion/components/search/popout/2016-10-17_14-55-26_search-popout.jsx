...
import toCapitalCase from 'components/search/toCapitalCase.jsx'
...
export default class ASearchPopout extends Component {
  ...
  render() {
    ...
    // TODO Localization
    let dataNav = [
      { link: <Link to={`/search/${query}/all`}>{this.trans.all && toCapitalCase(this.trans.all)}</Link>, content: <SearchAll videos={videos} lives={lives} playlists={playlists} users={users} query={query}/> },
      { link: <Link to={`/search/${query}/videos`}>{this.trans.videos && toCapitalCase(this.trans.videos)}</Link></Link>, content: <SearchVideos videos={videos} /> },
      { link: <Link to={`/search/${query}/channels`}>{this.trans.channels && toCapitalCase(this.trans.channels)}</Link></Link>, content: <SearchChannels users={users} /> },
      { link: <Link to={`/search/${query}/playlists`}>{this.trans.playlists && toCapitalCase(this.trans.playlists)}</Link></Link>, content: <div>playlists</div> },
    ]
