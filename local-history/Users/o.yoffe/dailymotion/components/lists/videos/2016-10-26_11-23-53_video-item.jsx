...
class VideoItem extends Component {
  ...
  render() {
    if(this.props.hasOwnProperty('is_on_air')){
        let live = true
        let { query, isPlaylist, className, owner, title, name, duration, thumbnails, views_total, created_date, html_url, videos_total, content_type, thumbnail_360_url } = this.props.video
    }else {
        let live = false
        let { query, isPlaylist, className, owner, title, name, duration, thumbnails, views_total, created_date, html_url, videos_total, content_type, thumbnail_360_url } = this.props
    }

    let thumbnail = thumbnails ? thumbnails.x360 : thumbnail_360_url
    const username = owner ? owner.username : this.props['owner.url'].substr(this.props['owner.url'].lastIndexOf('com/') + 3)
    const fullname = owner ? owner.fullname : this.props['owner.screenname']
    ...
    if (isPlaylist) {
      ...
      caption = live ? <div className="video-item__thumb__live">{this.trans.Live && this.trans.Live.toUpperCase()}</div> : <div className="video-item__thumb__duration">{ duration }</div>
    }
