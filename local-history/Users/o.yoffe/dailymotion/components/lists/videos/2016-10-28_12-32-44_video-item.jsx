...
window.moment
    ...
    return (
      <div className={classNames.join(' ')} >
        ...
        <a href={html_url} onClick={saveSearch.bind(null, query)} className="video-item__thumb">
          { caption }
          <img src={thumbnail} />
        </a>
        ...
        <div className="video-item__description">
          ...
          <h2 className="video-item__description__title">
            <a href={html_url} onClick={saveSearch.bind(null, query)}  >
              <TextClamp clamp={2}>{ title || name }</TextClamp>
            </a>
          </h2>
          <p className="video-item__description__channel">
            {this.trans.by} <a href={"/"+username} onClick={saveSearch.bind(null, query)}  >{ fullname }<VerifiedBadge verified={content_type === "verified-partner"} /></a>
          </p>
          { !isPlaylist ?
